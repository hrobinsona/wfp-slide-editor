...
- An inline formatting run mixed into its parent's own text (`<b>`, `<strong>`, `<em>`, `<a>`, a colour `<span>` — computed `display: inline` inside a text-bearing parent) is not a selectable element. Click, drag, and double-click on it resolve to the nearest ancestor that is not such a run, so bolding words never creates a second box inside the paragraph. Inline elements that are not mixed into text (a lone `<strong>` stat in a card, a chip `<span>`) stay selectable.
