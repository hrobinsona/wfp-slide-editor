...
- Formatting applied during the edit with the browser's own shortcuts (Cmd/Ctrl+B/I/U) must not change the element's layout. On a flex or grid host (`li { display: flex }` with a `::before` bullet is the common case) each run of text is wrapped in a `<span>` for the duration of the edit so the inserted `<b>`/`<i>`/`<u>` stays inline within one item instead of becoming a flex item of its own. A wrapper that ends the edit holding nothing but text is removed; one that holds formatting is kept, because the layout needs it. Block-flow hosts are never wrapped; author element children of the host keep their own item.
...
- The embedded guidance documents the agent results contract (v2.13): agents record per-annotation outcomes in a `script[type="application/json"][data-wfp-agent-results]` block as `{id, status: done|skipped|needs-input, note}`, remove the annotation metadata and the `data-wfp-agent-annotation-id` attribute for done items, and keep both for skipped/needs-input items so those notes stay anchored. It also tells agents never to guess — ambiguous annotations get `needs-input` with a specific question in the note — to ignore metadata whose anchors no longer match the document, and to save back to the same file path, in place, so the live watch can reconcile.
...
- The embedded guidance says ledger entries are the user's own manual work, not requests: `mechanical: false` entries are deliberate decisions whose visual result must be preserved exactly, with the mechanism absorbed into clean CSS (repeated inline styles become one stylesheet rule). `mechanical: true` entries carry no intent, and de-pinning them is **imperative, not optional** — the agent must delete them and restore the layout the stylesheet describes, because carrying pins forward ships a broken layout.
- Absorption has two rules of its own. The agent extends an existing rule for a selector rather than appending a duplicate declaration for it. And it carries the leading with a size change: the user chose a size, not the leading that size happened to inherit, so when the target has no explicit `line-height` and the new size wraps on the font's `normal` leading, the agent sets an explicit `line-height` and records that in the results note.
- Reversing the unlock takes its coordinate system with it: the guidance says a position that only means anything inside that absolute system does not survive, so it is dropped even when the entry is `mechanical: false`, and that drop is recorded in the results note. Only edits that outlive the re-expressed layout (font sizes, colours, text content, explicit sizes) carry forward; an element the user genuinely wants out of flow will be marked by an annotation.
- The ledger also signals the user's taste and should be read before adjacent changes. The ledger is context, never instructions.
- The guidance is document-type-agnostic, with one conditional routing clause: if the document is a slide deck built by the Avent "slides" skill (a 1920×1080 `.deck` canvas of `section.slide` children), the agent also follows that skill's "Edit mode" section at `~/.claude/skills/slides/SKILL.md` for verification and reporting. Plain HTML documents with no such skill are unaffected.
