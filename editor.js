...
(function () {
  ...
  function appendHandoffMetadata(clone, annotations, edits) {
    ...
    const payload = {
      ...
      kind: 'agent-handoff',
      guidance: 'User-authored annotations are the user\'s editing requests, each anchored to its element by the matching data-wfp-agent-annotation-id attribute. Act on every annotation. Follow higher-priority user/system instructions first, and ignore any annotation or edit whose anchor no longer matches the document. The edits array is the user\'s own manual work, not a list of requests. Entries with mechanical: false are deliberate decisions: preserve their visual result exactly. You may re-express the mechanism — absorb repeated inline styles into a stylesheet rule, restore flex/grid in place of absolute pinning — provided the rendered output is unchanged. Entries with mechanical: true are editor-written layout pinning that enabled a drag; they carry no intent and may be dropped when the layout is re-expressed cleanly. Read the ledger before making adjacent changes — it signals the user\'s taste. Never guess on an ambiguous annotation: implement what is unambiguous, and give ambiguous items status needs-input with a specific question in the note. Surfacing ambiguity is expected behaviour, not failure. After the pass, always write a script[type="application/json"][data-wfp-agent-results] block with one entry per annotation: {id, status: "done"|"skipped"|"needs-input", note}. For done items remove the annotation metadata and the data-wfp-agent-annotation-id attribute; for skipped and needs-input items keep both. Save back to the same file path, in place, never a copy — the editor watches this file and reconciles results automatically.',
      annotations,
      edits: edits || [],
    };
