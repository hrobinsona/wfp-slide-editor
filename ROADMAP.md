...
**Decided and shipped.** Of the options considered — a generated single file, multiple plain scripts loaded in a documented order, or one file with stronger internal boundaries — the first was chosen: `src/editor/*.js` fragments concatenated by `npm run build:editor` into one deployed `editor.js`, with `node scripts/build-editor.js --check` guarding drift. The bookmarklet, hosting, and zero-dependency runtime are unchanged. See `DESIGN.md` → Why Concatenation, Not a Bundler.
