...
Selectable targets are boxes, not text runs (v2.25). `findSelectableTarget()` climbs out of any element whose computed `display` is `inline` and whose parent carries its own text nodes — the shape the browser's native Cmd/Ctrl+B leaves behind (`<b>` among a paragraph's words), and the shape authors use for `<strong>`, `<em>`, `<a>` and the editor's own colour `<span>`. Treating such a run as an element gave it a ring and, on drag, an `unlockToAbsolute` that pulled the run out of the paragraph's flow while pinning the paragraph around it. The climb is deliberately narrow: inline-block/flex items and inline elements sitting in a whitespace-only parent are boxes an author placed and remain selectable.
