...
Selectable targets are boxes, not text runs (v2.25). `findSelectableTarget()` walks up through every ancestor whose computed `display` is `inline` — the shape the browser's native Cmd/Ctrl+B leaves behind (`<b>` among a paragraph's words), the shape authors use for `<strong>`, `<em>`, `<a>`, and the editor's own colour `<span>` wrapping any of them — and decides at the first non-inline ancestor: a block that carries its own text nodes is the target, otherwise the outermost run is a box the author placed (a chip `<span>` in a whitespace-only row) and is selected as before. Treating a run as an element gave it a ring and, on drag, an `unlockToAbsolute` that pulled it out of the paragraph's flow while pinning the paragraph around it. The climb is deliberately narrow: inline-block and flex/grid items never qualify, and replaced elements (`<img>`, inline `<svg>`, `<video>`) compute to `inline` but are exempt because they are boxes the user moves and resizes.
...
- Inline text edit on a flex/grid host (v2.26): the browser's native formatting shortcuts insert elements, and every element child of a flex container is a flex item, so a `<b>` around one word turned a bullet's sentence into two columns. `startTextEdit` wraps each run of text nodes in a `<span data-wfp-text-run>` after the history snapshot is taken and before the caret lands; `endTextEdit` unwraps any wrapper still holding only text (and normalises the split text nodes) before the after-snapshot. A wrapper holding formatting stays, marker included. The marker is deliberately outside the swept `data-wfp-edit-*` namespace — it is structural markup the deck now depends on, not editor state — so export keeps it and the editor recognises it on the next load: `isTextBearing` counts a wrapper as the host's own text (otherwise every inspector text control, all gated on it, would silently no-op during the edit), and `findSelectableTarget` resolves a wrapper to its host. History is wrapper-neutral: `snapshotElement` serialises through `textRunNeutralHtml`, so an inspector commit mid-edit — which ends and reopens the text-edit transaction while wrappers are installed — records neither the wrapper nor a content-free entry. This is a deliberate, scoped exception to "editor-injected DOM lives inside `#wfp-editor-root`": the wrapper has to sit in slide content to affect layout, and a formatted edit permanently adds an author-visible `<span>` (which author structural selectors such as `li > span` can then match).
